(defun ricorsiva-plus (regexp initial final)
  (list
   (nfa-regexp-comp-tot initial
			(list 'seq regexp (list 'star regexp))final)))
...
(defun ricorsiva-star (re initial final) 
  ((lambda (initialStar finalStar)
     (list 
      ...
      (if (atom (second re))
	  (nfa-regexp-comp-tot initialStar (list (second re)) finalStar)
	(nfa-regexp-comp-tot initialStar (second re) finalStar))))
   (gensym "q")  (gensym "q")))
...
(defun get-initial (nfa)
  (if (eq (car (car nfa)) 'iniziale) (car nfa)
    (get-initial (cdr nfa))))

(defun get-final (nfa)
  (if (eq (third (car nfa)) 'finale) (car nfa)
    (get-final (cdr nfa))))
